...
/* Plugin info */
#define UPDATE_URL				"https://github.com/skettios/TourneyBot"
#define WM_VERSION				"1.0b1"
...
{	
	...
	g_h_match_config = CreateConVar("wm_match_config", "tourneybot/ruleset_mr15.cfg", "Sets the match config to load on Live on 3");
	g_h_end_config = CreateConVar("wm_reset_config", "tourneybot/on_match_end.cfg", "Sets the config to load at the end/reset of a match");
	g_h_warmup_config = CreateConVar("wm_warmup_config", "tourneybot/ruleset_warmup.cfg", "Sets the config to load up for warmup");
	g_h_prac_config = CreateConVar("wm_prac_config", "tourneybot/prac.cfg", "Sets the config to load up for practice");
	g_h_half_time_config = CreateConVar("wm_half_time_config", "tourneybot/on_match_half_time.cfg", "Sets the config to load at half time of a match (including overtime)");
	...
	g_h_save_file_dir = CreateConVar("wm_save_dir", "tourneybot", "Directory to store SourceTV demos and TourneyBot logs");
	g_h_prefix_logs = CreateConVar("wm_prefix_logs", "1", "Enable or disable the prefixing of \"_\" to uncompleted match SourceTV demos and TourneyBot logs", FCVAR_NOTIFY);
	g_h_warmup_respawn = CreateConVar("wm_warmup_respawn", "0", "Enable or disable the respawning of players in warmup", FCVAR_NOTIFY);
	g_h_status = CreateConVar("wm_status", "0", "TourneyBot automatically updates this value to the corresponding match status code", FCVAR_NOTIFY);
	...
	g_h_chat_prefix = CreateConVar("wm_chat_prefix", "TourneyBot", "Change the chat prefix. Default is TourneyBot", FCVAR_PROTECTED);
	...
	g_h_t_score = CreateConVar("wm_t_score", "0", "TourneyBot automatically updates this value to the Terrorist's total score", FCVAR_NOTIFY);
	g_h_ct_score = CreateConVar("wm_ct_score", "0", "TourneyBot automatically updates this value to the Counter-Terrorist's total score", FCVAR_NOTIFY);
//	g_h_notify_version = CreateConVar("wm_version_notify", WM_VERSION, WM_DESCRIPTION, FCVAR_NOTIFY|FCVAR_REPLICATED);
	...
	g_h_competition = CreateConVar("wm_competition", "TourneyBot", "Name of host for a competition. eg. ESEA, Cybergamer, CEVO, ESL", FCVAR_PLUGIN);
...
public Action:Unpause(client, args)
{
	if (g_paused)
	{
		if (GetConVarBool(g_h_pause_confirm))
		{
			...
			{
				g_pause_offered_ct = true;
				PrintToConsoleAll("<TourneyBot> CT have asked to unpause the game. Please type /unpause to unpause the match.");
			...
			{
				g_pause_offered_t = true;
				PrintToConsoleAll("<TourneyBot> T have asked to unpause the game. Please type /unpause to unpause the match.");
			...
			{
				PrintToConsole(client, "<TourneyBot> You must be on T or CT to enable /unpause");
	...
	else
	{
		...
		PrintToConsole(client,"<TourneyBot> Server is not paused or was paused via rcon");
	}
}
...
public Action:ForceAllReady(client, args)
{
	...
	if (g_ready_enabled)
	{
		...
		{
			PrintToConsole(client, "<TourneyBot> %T", "Forced Ready", LANG_SERVER);
		}
	...
	else
	{
		...
		{
			PrintToConsole(client, "<TourneyBot> %T", "Ready System Disabled2", LANG_SERVER);
		}
	}
...
public Action:LastMatch(client, args)
{
	...
	{
		PrintToConsole(client, "<TourneyBot> No Matches Played");
	}
	return Plugin_Handled;
}
...
stock LiveOn3Override()
{
	...
	ServerCommand("mp_teamname_1 %s", g_ct_name);
	ServerCommand("mp_teamname_2 %s", g_t_name);
	PrintToChatAll("\x01 \x09[\x04%s\x09]\x01 %s", CHAT_PREFIX, "Restart 1/3...");
	CreateTimer(4.0, Restart2);
	g_t_pause_count = 0;
	g_ct_pause_count = 0;
...
{
	PrintToChatAll("\x01 \x09[\x04%s\x09]\x01 %s", CHAT_PREFIX, "Restart 2/3...");
	ServerCommand("mp_restartgame 1");
	CreateTimer(3.0, Restart3);
}

public Action:Restart3(Handle:timer)
{
	PrintToChatAll("\x01 \x09[\x04%s\x09]\x01 %s", CHAT_PREFIX, "Restart 3/3...");
	ServerCommand("mp_restartgame 5");
	CreateTimer(5.1, LiveOn3Text);
}
...
public Action:AdvertGameTech(Handle:timer, any:client)
{
	...
	{
		PrintToChat(client, "\x01 \x09[\x04%s\x09]\x01 %t \x03TourneyBot", CHAT_PREFIX, "Powered By");
	}
}
...
public Action:AdvertGameTechSpecs(Handle:timer)
{
	for (new i = 1; i <= MaxClients; i++)
	{
		...
		{
			PrintToChat(i, "\x01 \x09[\x04%s\x09]\x01 %t \x03TourneyBot", CHAT_PREFIX, "Powered By");
		}
	}
}
...
{
	PrintToChatAll("\x01 \x09[\x04%s\x09]\x01 %s", CHAT_PREFIX, "Knife Round to Choose Sides");
	ServerCommand("mp_warmup_end");
	ServerCommand("mp_restartgame 3");
	CreateTimer(5.1, KnifeOn3Text);
...
IsActive(client, bool:silent)
{
	...
	else
	{
		if (!silent)
		{
			if (client != 0)
			{
				PrintToChat(client, "\x01 \x09[\x04%s\x09]\x01 %t", CHAT_PREFIX, "TourneyBot Inactive");
			}
			else
			{
				PrintToServer("\x01 \x09[\x04%s\x09]\x01 %t", CHAT_PREFIX, "TourneyBot Inactive", LANG_SERVER);
			}
		}
	}
	return false;
...
IsAdminCmd(client, bool:silent)
{
	...
	else
	{
		if (!silent)
		{
			PrintToChat(client, "\x01 \x09[\x04%s\x09]\x01 %t", CHAT_PREFIX, "TourneyBot Rcon Only");
		}
	}
	return false;
}
...
public Action:SayChat(client, args)
{
	...
	else if (message[0] == '!' || message[0] == '.' || message[0] == '/')
	{
		...
		if (StrEqual(command, "ready", false) || StrEqual(command, "rdy", false) || StrEqual(command, "r", false) || StrEqual(command, "gaben", false))
		{
			ReadyUp(client);
		}
...
Handle:MySQL_Connect()
{
	...
	{
		LogError("No TourneyBot database configuration found (note: case-sensitive)!");
	}
...
public Action:CreateTable(client, args)
{
	...
	if (success)
	{
		ReplyToCommand(client, "TourneyBot results table creation successful!");
	}
...
public Action:WMVersion(client, args)
{
	if (client == 0)
	{
		PrintToServer("\"wm_version\" = \"%s\"\n - <TourneyBot> %s", WM_VERSION, WM_DESCRIPTION);
	}
	else
	{
